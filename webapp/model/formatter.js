sap.ui.define([], function () {
	...
	return {
		...
		CopyButtonVisibility: function(status){ // added by uml6kor 20/7/2021 for copy button visibility.
			if (status !== null && (status.indexOf("FF001") !== -1 )) {
				return true;
			} else {
				return false;
			}
		...
		HideTile: function (role) {
			if (role[0].role === "HRREP") {
				return true;
			} else {
				return false;
			}
		},
		onTicketAmount: function (amount) {
		...
		ModelNameFormat: function (val) {
			if (val !== "" && val !== null) {
				val = val.trim();
				var asgData = sap.ui.getCore().getModel("AsgModelData").getData();
		...
		EditableCopyFields:function(val){
			if(val === true){
				return true;
			}
			else{
