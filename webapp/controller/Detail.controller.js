sap.ui.define([
	...
	"sap/ui/model/Sorter"
], function (BaseController, MessageBox, JSONModel, Filter, MessageToast, formatter, ColumnListItem, Input, DatePicker, FilterOperator,
	Sorter) {
	"use strict";

	return BaseController.extend("com.bosch.hr.swift_trv.controller.Detail", {
		formatter: formatter,
		onInit: function () {
				...
				cells: [
					...
					new Input({
						...
						valueHelpRequest: function (oEvent) {
							var catId = that.getView().getModel("detail").getData().ZZ_CATID;
							if (catId !== undefined) {
								sap.ui.core.BusyIndicator.show(-1);
								that.selectedPath = oEvent.getSource().getParent().getBindingContextPath();
								that.FundServiceCall(that.selectedPath);
							} else {
								that.getView().getModel("detail").getData().view.ZZ_CATID_ERROR = "Error";
								that.getView().getModel("detail").refresh();
								sap.m.MessageToast.show("Please select Travel Classification ..!");
							}
						},
						...
						valueHelpRequest: function (oEvent) {
							...
							that.selectedFund = that.getView().getModel("detail").getProperty("/TRV_HDRtoTRV_COST_ASGN/results/0").ZZ_GEBER;
		...
		onChangeTravelClassification: function (evt) {
			this.getView().getModel("detail").getData().TRV_HDRtoTRV_COST_ASGN.results[0].ZZ_GEBER = "";
			this.getView().getModel("detail").refresh();
		},
			...
			if (that.bFundServiceCall === true) {
				...
				this.oDataModel.read("Fund_F4_Help?ZZ_BEGDA='" + Data.ZZ_DATV1 + "'&ZZ_ENDDA='" + Data.ZZ_DATB1 +
					...
					function (oData, response) {
						...
						var FundData = data.d.results;
						var Fund_F4_Help = [];
						var ZZ_CATID = that.getView().getModel("detail").getData().ZZ_CATID;
						for (var i = 0; i < FundData.length; i++) {
							if (ZZ_CATID === "GLOB" && (FundData[i].fincode === "F08" || FundData[i].fincode === "F09")) {
								Fund_F4_Help.push(FundData[i]);
							} else if (ZZ_CATID !== "GLOB" && (FundData[i].fincode !== "F08" && FundData[i].fincode !== "F09")) {
								Fund_F4_Help.push(FundData[i]);
							}
						}
						that.getView().getModel("detail").getData().Fund_F4_Help = Fund_F4_Help;
						that.getView().getModel("detail").refresh();
						if (Fund_F4_Help.length === 0) {
							sap.m.MessageToast.show("No Data Found");
						} else {
							that.bFundServiceCall = false; ////// replaced false with true
							that._oDialogFund.open();
						}
						sap.ui.core.BusyIndicator.hide();
					},
		...
		WBSServiceCall: function (path) {
			...
			if (that.bWBSServiceCall === true) {
				...
				this.oDataModel.read("WBSf4Help?" + "ZZ_REINR='" + this.reqNo + "'&ZZ_PERNR='" + this.Perner + "'&ZZ_LAND1='" + Data.ZZ_LAND1 +
					...
					function (oData, response) {
						var data = JSON.parse(response.body);
						var wbsElementData = data.d.results;
						var WBSF4Help = [];
						for (var i = 0; i < wbsElementData.length; i++) {
							if (that.selectedFund === "F08" && (wbsElementData[i].ZZ_CATG === "4" || wbsElementData[i].ZZ_CATG === "04")) {
								WBSF4Help.push(wbsElementData[i]);
							} else if (that.selectedFund !== "F08" && (wbsElementData[i].ZZ_CATG !== "4" || wbsElementData[i].ZZ_CATG !== "04")) {
								WBSF4Help.push(wbsElementData[i]);
							}
						}
						that.getView().getModel("detail").getData().WBSF4Help = WBSF4Help;
						that.getView().getModel("detail").refresh();
						if (data.d.results.length === 0) {
							sap.m.MessageToast.show("No Data Found");
						} else {
		...
		_onProductMatched: function (oEvent) {
			...
			if (this.aData.isCreate === true) {
				...
				this.oDataModel.submitBatch(function (oResult) {
					...
					if (that.aData.isCopy === true) {
						//that.promiseCopy = jQuery.Deferred();
						that.promiseCopy.resolve();
					}
					sap.ui.getCore().getModel("products").refresh();
		...
		onPressCopy: function (evt) {
			var that = this;
			if (that.aData.isCopy !== true) {
				MessageBox.confirm("You are creating a new business travel request from the current request. Do you want to Continue?", {
					actions: ["Continue", MessageBox.Action.CANCEL],
					emphasizedAction: "Continue",//MessageBox.Action.OK,
					onClose: function (sAction) {
						if (sAction === "Continue") {
							that.aData.isCopy = true;
							that.promiseCopy = jQuery.Deferred();
							that.onEditToggleButtonPress();
							$.when(that.promiseCopy).done(function () {
								that.onCountryFromChange();
								that.onCountryToChange();
								that.getView().getModel("detail").getData().ZZ_DEP_REQ = "0000000000";
								that.getView().getModel("detail").getData().ZZ_REINR = "0000000000";
								that.getView().getModel("detail").refresh();
								that.getView().byId("idCopyDetButton").setVisible(false);
								that.reqNo = "0000000000";
							});
						}
					}
				});
		...
		onEditToggleButtonPress: function () {
			...
			if (!bCurrentShowFooterState === true || this.aData.isCopy === true) {
				this.getView().getModel("products").getData().editable = true;
				this.getView().getModel("products").refresh();
		...
		handleFullScreen: function () {
			...
			this.oRouter._oRoutes.detail._oConfig.layout = "MidColumnFullScreen";
		...
		handleExitFullScreen: function () {
			this.MinScreen = true;
			this.destroyUIElements();
			this.oRouter._oRoutes.detail._oConfig.layout = "TwoColumnsMidExpanded";
		...
		onTicketing: function (oEvent) {
			...
			this.getOwnerComponent().getHelper().then(function (oHelper) {
				...
				//	that.oRouter._oRoutes.Ticketing._oConfig.layout = "MidColumnFullScreen";
				that.oRouter.navTo("Ticketing", {
		...
		onCreateInsurance: function (oEvent) {
			...
			this.getOwnerComponent().getHelper().then(function (oHelper) {
				oNextUIState = oHelper.getNextUIState(1);
				//	that.oRouter._oRoutes.Insurance._oConfig.layout = "MidColumnFullScreen";
		...
		_handleValueHelpClose_fund: function (evt) {
			...
			this.selectedFund = selectedItem;
			this.getView().getModel("detail").getProperty(this.selectedPath).ZZ_FIPOS = "";
		...
		onPressSave: function (sAction) {
			...
			var aData = this.getView().getModel("detail").getData();
			//uml6kor 20/7/2021 copy save
			var aGlobData = sap.ui.getCore().getModel("global").getData();
			if (aGlobData.isCopy) {
				aData.ZZ_DEP_REQ = '0000000000';
